...
from eventify.cli import _events_to_frame, app
...
def test_convert_output_has_correct_frame_count(synthetic_video, tmp_path):
    out = tmp_path / "out.mp4"
    runner.invoke(app, ["convert", str(synthetic_video), str(out)])
...
def test_convert_nonexistent_input_returns_error(tmp_path):
    out = tmp_path / "out.mp4"
    result = runner.invoke(app, ["convert", str(tmp_path / "missing.avi"), str(out)])
    assert result.exit_code != 0


def test_export_produces_hdf5(synthetic_video, tmp_path):
    out = tmp_path / "events.h5"
    result = runner.invoke(app, ["export", str(synthetic_video), str(out)])
    assert result.exit_code == 0
    assert out.exists()
...
def test_export_sensor_size_override(synthetic_video, tmp_path):
    out = tmp_path / "events.h5"
    runner.invoke(app, ["export", str(synthetic_video), str(out), "--sensor-size", "16,16"])
    with h5py.File(str(out), "r") as f:
...
def test_export_sensor_size_invalid(synthetic_video, tmp_path):
    out = tmp_path / "events.h5"
    result = runner.invoke(app, ["export", str(synthetic_video), str(out), "--sensor-size", "bad"])
    assert result.exit_code != 0


def test_export_interp_produces_events(synthetic_video, tmp_path):
    out = tmp_path / "interp.h5"
    result = runner.invoke(app, ["export", str(synthetic_video), str(out), "--interp", "3"])
    assert result.exit_code == 0
    with h5py.File(str(out), "r") as f:
...
def test_export_nonexistent_input_raises(tmp_path):
    out = tmp_path / "events.h5"
    result = runner.invoke(app, ["export", str(tmp_path / "missing.avi"), str(out)])
    assert result.exit_code != 0
